...
def _openai_compatible_chat_completions_url(base_url: str) -> str:
    url = _normalize_model_http_url(base_url).rstrip("/")
    if url.endswith("/chat/completions"):
        return url
    return f"{url}/chat/completions"


def _api_key_from_env(api_key_env: str, *, required: bool) -> str:
    env_name = api_key_env.strip()
    if not env_name:
        if required:
            raise RuntimeError("Missing API key env var name")
        return ""
...
def _json_headers_with_optional_bearer(api_key: str) -> dict[str, str]:
...
def _openai_compatible_model_candidates(backend: JudgeBackend, model: str) -> list[str]:
    model = model.strip()
    candidates = [model or "llm"]
    if backend == "vllm" and model.lower() == "qwen/qwen3.5-35b-a3b":
        candidates.append("llm")
    return list(dict.fromkeys(candidate for candidate in candidates if candidate))


def _should_retry_vllm_model_alias(error: Exception) -> bool:
    ...
    return "model" in text and any(marker in text for marker in missing_model_markers)
...
def _openai_compatible_response_text(body: dict[str, Any]) -> str:
...
def _call_openai_compatible_chat_model_messages(
    *,
    ...
    response_json: bool,
    max_output_tokens: int | None,
    open_json: Callable[[urllib.request.Request], dict[str, Any]],
) -> str:
    url = _openai_compatible_chat_completions_url(base_url)
    api_key = _api_key_from_env(api_key_env, required=backend == "openrouter")
    model_candidates = _openai_compatible_model_candidates(backend, model)
    ...
    for i, candidate_model in enumerate(model_candidates):
        ...
        req = urllib.request.Request(
            ...
            headers=_json_headers_with_optional_bearer(api_key),
            method="POST",
        )
        try:
            body = open_json(req)
            return _openai_compatible_response_text(body)
        except RuntimeError as e:
            last_error = e
            has_fallback = i + 1 < len(model_candidates)
            if backend == "vllm" and has_fallback and _should_retry_vllm_model_alias(e):
                continue
            raise

    if last_error is not None:
        raise last_error
    raise RuntimeError("No OpenAI-compatible model candidates were available")
...
def call_chat_model_messages(
    *,
    ...
    if backend in {"openrouter", "vllm"}:
        return _call_openai_compatible_chat_model_messages(
            backend=backend,
            model=model,
            base_url=base_url,
            ...
            response_json=response_json,
            max_output_tokens=max_output_tokens,
            open_json=urlopen_json_with_retry,
        )

    if backend == "gemini":
        api_key = _api_key_from_env(api_key_env, required=True)
        model_path = model if model.startswith("models/") else f"models/{model}"
        url = f"{_normalize_model_http_url(base_url).rstrip('/')}/{model_path}:generateContent"
...
class BuilderFilesystemTools:
    ...
    async def read_many(
        self,
        reads: Annotated[list[dict[str, Any]], "List of read specs: {path, optional start_line, optional num_lines}. At most 4 reads per call."],
    ) -> ToolResult:
...
class SyntheticFilesystemReward:
    ...
    async def _score_answerer_output(
        ...
        *,
        ...
        # Do not let strict final-answer formatting gate semantic judging.
        # If exact match fails in hybrid/LLM mode, the judge should see the
        # answerer's text even when it was not emitted as `Answer: ...`.
        judge_candidate = candidate if candidate else "Unknown"
        exact_match = 0.0
        judge_used = 0.0
        judge_score = 0.0
        judge_error = 0.0
        ...
        if terminal_step < 1.0:
            ...
            if self.reward_mode == "exact":
                ...
                judge_used = 1.0
                try:
                    judge_score = await self._llm_judge_score(judge_candidate)
                except Exception:
    ...
    async def __call__(self, history: list[Message]) -> tuple[float, dict[str, float]]:
        ...
        if terminal_step >= 1.0 and answerer_evaluated >= 1.0:
            self.last_answerer_trace = {
                "answer_text": str(answerer_result.get("answer_text", ""))[:4000],
                "read_paths": [str(path) for path in answerer_result.get("read_paths", [])],
                "steps": float(answerer_result.get("steps", 0.0) or 0.0),
                "used_raw_docs": answerer_result.get("used_raw_docs", False),
                "used_readme": answerer_result.get("used_readme", False),
                "freeform_final_answer": answerer_result.get("freeform_final_answer", False),
                "prefixed_final_answer": answerer_result.get("prefixed_final_answer", False),
                "answerer_repeats": int(answerer_repeats_used),
            }
        metrics.update(
            {
        ...
        *,
        ...
        if backend in {"openrouter", "vllm"}:
            return _call_openai_compatible_chat_model_messages(
                ...
                response_json=response_json,
                max_output_tokens=max_output_tokens,
                open_json=self._urlopen_json_with_retry,
            )
...
class RedactingAgentToolMessageEnv(MessageEnv):
    ...
    async def step(self, message: Message) -> MessageStepResult:
        ...
        if done:
            ...
            answerer_trace = getattr(self.reward_fn, "last_answerer_trace", None)
            if self.log_step_details and answerer_trace:
                logs["answerer_trace"] = json.dumps(answerer_trace, ensure_ascii=False)
...
class SyntheticFilesystemEnvGroupBuilder(EnvGroupBuilder):
    ...
    builder_executor_backend: JudgeBackend = "vllm"
...
class SyntheticFilesystemRLDataset(RLDataset):
    def __init__(
        self,
        env_group_builders: list[SyntheticFilesystemEnvGroupBuilder],
        batch_size: int,
        *,
        num_epochs: int = 1,
        ...
        self.seed = seed
        self._epoch_cache: dict[int, list[SyntheticFilesystemEnvGroupBuilder]] = {}

    def _batches_per_epoch(self) -> int:
        return (len(self.env_group_builders) + self.batch_size - 1) // self.batch_size

    def _builders_for_epoch(self, epoch_index: int) -> list[SyntheticFilesystemEnvGroupBuilder]:
        if not self.reshuffle_each_epoch:
            return self.env_group_builders
        if epoch_index not in self._epoch_cache:
            builders = list(self.env_group_builders)
            random.Random(self.seed + epoch_index).shuffle(builders)
            self._epoch_cache[epoch_index] = builders
        return self._epoch_cache[epoch_index]

    def get_batch(self, index: int) -> Sequence[EnvGroupBuilder]:
        batches_per_epoch = max(1, self._batches_per_epoch())
        epoch_index = index // batches_per_epoch
        batch_index_in_epoch = index % batches_per_epoch
        epoch_builders = self._builders_for_epoch(epoch_index)
        start = batch_index_in_epoch * self.batch_size
        end = start + self.batch_size
        batch_builders = epoch_builders[start:end]
        examples_this_batch = len(batch_builders)
        examples_seen = epoch_index * len(self.env_group_builders) + start + examples_this_batch
            ...
            sorted(
                {
                    ...
                    "training_epoch": float(epoch_index)
                    + float(batch_index_in_epoch + 1) / float(batches_per_epoch),
                    "training_epoch_index": float(epoch_index),
                    "training_epochs_total": float(self.num_epochs),
...
class SyntheticFilesystemDatasetBuilder(RLDatasetBuilder):
    ...
    async def __call__(self) -> tuple[RLDataset, RLDataset | None]:
        ...
        train_dataset = SyntheticFilesystemRLDataset(
            train_builders,
        ...
        eval_dataset = SyntheticFilesystemRLDataset(
            eval_builders,
            batch_size=self.batch_size,
            num_epochs=1,
